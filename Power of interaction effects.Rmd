...
This function stores the results of the simulation in a dataframe called `res`.
...
<br>
<img src="Table1.png" height="150px" width="600px" >


Because the relevant effect size for moderation (Beta 3) only slightly changes as the correlation changes, we report only one value of rho (r = .30) in subsequent analyses. The effect sizes for the interaction term seem to have a realistic and relevant range. Values smaller than .15 are not likely to have much practical relevance. Larger effect size values than 0.35 do not occur very often, and if they do occur it will not be problematic to detect them.
After generating data according to the specifications set above, we used the lm() function in R to perform the analyses. The number of replications in each condition was set 2,000.  The results are shown in Figure 1 for alpha = .05 and in Figure 2 for alpha = .01.
...
```{r simulation1, eval=F, echo=TRUE}
...
p <- ggplot(data=res, aes(y=power, x=N, group=effectSize, colour=effectSize)) + geom_point(size=2) + geom_line(size=1)
...
fig1 <- p + scale_color_viridis(discrete=TRUE) + theme_bw(base_size = 14)

print(fig1);

### Store to disk
ggsave(plot = fig1, filename="fig1.pdf");

```

From these analyses we can conclude that with relatively large effects (> 0.30) for the interaction term a sample of N = 50 is sufficient if you accept a type I error of 5%. For N = 100 the power is even more than 90%. When the interaction corresponds with a small effect size, which is far more common, N = 350 is necessary for a power of 84%. For alpha = 0.01 we rerun the simulation. 
...
res$effectSize <- ordered(res$e, levels=c(1,3,9), labels= c("0.31","0.23","0.15"))
...
p <- p + coord_cartesian(ylim=c(0.1, 1.0)) + scale_y_continuous(breaks=seq(0.10, 1, 0.10))
p <- p + coord_cartesian(xlim=c(40, 510)) + scale_x_continuous(breaks=seq(50, 500, 50) )
fig2 <- p + scale_color_viridis(discrete=TRUE) + theme_bw(base_size = 14)

print(fig2);

### Store to disk
ggsave(plot = fig2,filename="fig2.pdf")
