...
To run a proper simulation it is necessary to genrate data according to the right model.
Defining the right model is sometimes a challenge.
...
CheckPars has four input vectors: the correlation between the two predictors, the error levels, the auto-regression levels and the coefficients of the three predictor terms.
simPower.moderation has the same input and in addition a vector of sample sizes, the alpha level, and the number of replications.
...
This model is based on an experimental design with two groups. There two mediator variables which are measures five times. There is one moderator.
