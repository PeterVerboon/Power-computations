...
# anayses are with random intercept across subjects only (randomDay = FALSE) of across subjects and days
# author: P. Verboon, 
# date: october, 2016 adapted february, 2018


simPower.ESM <- function(nbeep, nday, nsubj, rho, sd.subj = 1, sd.day = 0, 
                         maxiter=1000, ntest=1, randomDay = FALSE, pMisDay = 0, pMisBeep = 0) 
  {   
  ...
  res <- matrix(data=0,nrow=maxiter, ncol=4)
  colnames(res) <- c("regression coefficient","Power", "sd_Subject", "sd_Day")
...
res <- simPower.ESM(nbeep = 10, nday = 14, nsubj = 25, 
                    sd.subj = 1, sd.day = 1, 
                    maxiter=100, rho = 0.15, 
                    ntest=4, randomDay = FALSE, 
                    pMisDay = .40, pMisBeep = .50)
