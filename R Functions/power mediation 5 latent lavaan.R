...
## 1 mediator
## latent growth model
## b-path  effect sizes of latent intercept and latent slope
...
showdist(dat = res$raw, var = "effect li", plot = TRUE)
showdist(dat = res$raw, var = "effect ls", plot = TRUE)

showdist(dat = res$raw, var = "indirect i", plot = TRUE)
showdist(dat = res$raw, var = "indirect s", plot = TRUE)

showdist(dat = res$raw, var = "power int", plot = TRUE)
...
## loop over sample sizes and effect sizes
 ...
 samSizes <- seq(from=200, to=400, by=50)
 esSizes <- c(.20,.50)
 ...
 save(out, file="modmedlat_alpha05_12sep2018.Rdata")
